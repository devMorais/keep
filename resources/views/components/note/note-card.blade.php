@foreach ($notes as $note)
    <div class="col-xxl-3 col-md-6 col-xl-4">
        <div class="single_note"
            @if ($note->appearance_type == 'color') style="background:{{ $note->color_name }}"
        @else
        style="background-image:url('{{ asset($note->image_path) }}')" @endif>
            ...
            <div class="ions_area">
                <ul>
                        ...
                        <div class="theme_area">
                            <ul class="theme_color">
                                ...
                                @foreach (config('appearance.colors') as $color)
                                    <li class="appearance" data-color="{{ $color }}" data-type="color"
                                        data-id="{{ $note->id }}"><a class="red"
                                            style="background: {{ $color }}" href="javascript:;"></a>
                                    </li>
                                @endforeach
                            </ul>
                            <ul class="theme_img">
                                ...
                                @foreach (config('appearance.images') as $image)
                                    <li class="appearance" data-image="{{ $image }}" data-type="image"
                                        data-id="{{ $note->id }}"><a style="background: url('{{ asset($image) }}')"
                                            class="img_2" href="#"></a>
                                    </li>
                                @endforeach
                            </ul>
                    ...
                    <li>
                        ...
                        <form class="delete-note-{{ $note->id }}" action="{{ route('notes.destroy', $note->id) }}"
                            method="POST">
                            @method('DELETE')
                            @csrf
    ...
    <div class="custom_modal_area" data-modal="modal_{{ $note->id }}">
        ...
        style="background-image:url('{{ asset($note->image_path) }}')" @endif>
            ...
            <form action="{{ route('notes.update', $note->id) }}" method="POST"
                class="update-note-{{ $note->id }}">
                @csrf
                @method('PUT')
                <input type="text" placeholder="Título" name="title" value="{{ $note->title }}">
                <textarea rows="4" placeholder="Nota" name="content" id="editorjs">{!! $note->content !!}</textarea>
            </form>
            <div class="ions_area">
                <ul>
                    <li>
                ...
                <a class="" href="javascript:;"
                    onclick="$('.update-note-{{ $note->id }}').submit()">Atualizar</a>
            </div>
        </div>
    </div>
@endforeach
